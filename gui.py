...
class CleanerGUI(tk.Frame):
    ...
    def create_widgets(self):
        # Directory selection
        tk.Label(self, text="Target Directory:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ...
        tk.Label(self, text="Extensions to Delete:").grid(row=1, column=0, padx=10, pady=5, sticky="w")

        for i, ext in enumerate(sorted(TEMPLATE_FILE_EXTENSIONS), start=2):
            checkbox = tk.Checkbutton(self, text=ext, variable=self.extension_vars[ext])
            checkbox.grid(row=i, column=1, padx=10, sticky="w")

        # Action button
        action_button = tk.Button(self, text="Run Cleaner", command=self.run_cleaner)
